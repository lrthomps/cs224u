...
def create_train_state(
    model: FlaxDistilBertForSequenceClassification,
...
def main():
    ...
    sm_config_dict["num_hidden_layers"] = 3

    sm_config = DistilBertConfig(**sm_config_dict)
    sm_model = FlaxDistilBertForSequenceClassification(sm_config)

    sm_model.params['distilbert']['embeddings'] = model.params['distilbert']['embeddings']
    sm_model.params['classifier'] = model.params['classifier']
    sm_model.params['pre_classifier'] = model.params['pre_classifier']
    for i in range(3):
        sm_model.params['distilbert']['transformer']['layer'][str(i)] = model.params['distilbert']['transformer']['layer'][str(i)]

    state = create_train_state(
        sm_model, learning_rate_fn, is_regression, num_labels=num_labels, weight_decay=training_args.weight_decay
    )
        ...
        for step, batch in enumerate(
            ...
            if (cur_step % training_args.save_steps == 0 and cur_step > 0) or (cur_step == total_steps):
                ...
                if jax.process_index() == 0:
                    ...
                    sm_model.save_pretrained(training_args.output_dir, params=params)
